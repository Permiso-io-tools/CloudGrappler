            ...
            with open('data/data_sources.json') as input_file:
                ...
                if source_type == '*':
                    for key, data_type in data_input.items():
                         service_type(data_type, new_query, json_output, key, start_date, end_date, file_size)
              
                else:
                    data_type = data_input.get(source_type.upper(), [])
                    service_type(data_type, new_query, json_output, source_type, start_date, end_date, file_size)
        
        elif add_file:
            data = load_json(add_file)
            if permiso_intel:
                intel(data)
            combined_data = {d["Query"]: d["Source"] for d in data}
      
            source_types = list(set(d["Source"].lower() for d in data))
            
      
            return combined_data, source_types
        else:
            data = load_json('data/queries.json')
...
def generic_function(combined_data, source_types, json_output, start_date, end_date, file_size):
    try:
        with open('data/data_sources.json') as input_file:
            data_input = json.load(input_file)
    ...
    for source_type in source_types:
        ...
        if clean_queries:
            data_source(data_input, clean_queries, source_type, json_output, start_date, end_date, file_size)
        else:
            print("No source type detected")

def data_source(data_input, queries, source_type, json_output , start_date, end_date, file_size):
    if source_type == '*':
     
        for key, data_type in data_input.items():
            service_type(data_type, queries, json_output, key, start_date, end_date, file_size)
    else:
...
def service_type(data_type, queries, json_output, source_type, start_date, end_date, file_size):
        ...
        if bucket:
            ...
            if containers != None:
                for container in containers:
                    run_process("azure", accountname, container, queries, json_output, source_type, start_date, end_date, file_size)
            else:
                    container = False
                    run_process("azure", accountname, container, queries, json_output, source_type, start_date, end_date, file_size)